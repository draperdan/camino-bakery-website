...
class LiveItemManager(models.Manager):
	def get_query_set(self):
		return super(LiveItemManager, self).get_query_set().filter(status=self.model.LIVE_STATUS).filter(is_available=True)
...
class Item(models.Model):
	...
	description = models.TextField(help_text='A brief description of the item. No HTML is allowed.', blank=True)
	one_off_price = models.DecimalField(max_digits=5, decimal_places=2, help_text="Use if item isn't available in different sizes. Amount cannot be larger than five digits. Don't use a dollar sign.", blank=True, null=True)
	status = models.IntegerField(choices=STATUS_CHOICES, default=DRAFT_STATUS)
	item_type = models.ForeignKey('ItemType')
	photo = models.ImageField(upload_to='images/menu/photos', width_field='width', height_field='height', help_text='Please use JPG or PNG formats. Will populate the width and height fields on save.', blank=True)
...
class Size(models.Model):
	size = models.CharField(max_length='100', help_text='Limited to 100 characters. Use either numerical or text-based characters.')
	price = models.DecimalField(max_digits=5, decimal_places=2, help_text="Amount cannot be larger than five digits. Don't use a dollar sign.")
	description = models.TextField(help_text='A brief description of the size. No HTML is allowed.', blank=True)
...
class ItemTypeGroup(Category):
	item_types = models.ManyToManyField(ItemType, help_text='Select the item types you want to group together.')
...
class BreadSchedule(models.Model):
	bread = models.ForeignKey(Item, limit_choices_to={'item_type__title': 'Bread'})
